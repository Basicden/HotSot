...
kafka_producer = KafkaProducer("search-service")
health_checker = HealthChecker("search-service")


@asynccontextmanager
async def lifespan(app: FastAPI):
    await init_service_db("search", Base.metadata)
...
setup_tracing("search-service", app)
...
async def readiness():
    db_ok = await health_checker.check_db("search")
