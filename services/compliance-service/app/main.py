"""HotSot Compliance Service — V2 Production-Grade."""
...
kafka_producer = KafkaProducer("compliance-service")
health_checker = HealthChecker("compliance-service")


@asynccontextmanager
async def lifespan(app: FastAPI):
    await init_service_db("compliance", Base.metadata)
    await redis_client.connect()
    await kafka_producer.start()
    health_checker.mark_ready(True)
    logger.info("compliance_service_started")
    yield
...
app = FastAPI(title="HotSot Compliance Service", version="2.0.0", lifespan=lifespan)
...
setup_tracing("compliance-service", app)
...
async def readiness():
    db_ok = await health_checker.check_db("compliance")
